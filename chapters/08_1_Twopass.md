...
The multi-pass approach is used in the address resolution logic of an _assembler_, a program that translates Duck Machine assembly
code to machine code.   In the first pass, the assembler
...
the assembler will translate the label `r1bigger` into a
...
the address resolver will need to count instructions
twice to determine the addresses.  In the first pass,
it creates a table associating labels like `r1bigger`
with addresses by counting the number of machine instructions
to be produced.  In the second pass, the assembler ocunts
machine instructions again, and also produces the
